...
const ALS_OPTIONS = Array.from({ length: 77 }, (_, i) => ({
  data: i * 25,
  label: (i * 25).toString(),
}));
const BRIGHTNESS_OPTIONS = Array.from({ length: 101 }, (_, i) => ({
  data: i,
  label: i.toString(),
}));
...
export default function ALSPanel() {
  ...
  const fetchAndSetPairs = async () => {
    ...
    if (Array.isArray(map)) {
      const pairs = map
        ...
        .map(([k, v]) => [
          ALS_OPTIONS.find(opt => opt.data === Number(k))?.data ?? ALS_OPTIONS[0].data,
          BRIGHTNESS_OPTIONS.find(opt => opt.data === Number(v))?.data ?? BRIGHTNESS_OPTIONS[0].data
        ] as [number, number]);
      setPairs(pairs);
    } else {
      setPairs([]);
  ...
  useEffect(() => {
    console.log('pairs updated:', pairs);
  }, [pairs]);
  ...
  const saveMap = async (): Promise<{ success: boolean } | undefined> => {
    ...
    setSaving(true);
    setError(null);
    console.log("Saving pairs:", pairs); // Log for debugging!
                ...
                <>
                  {pairs.length > 0 ? (
                    pairs.map(([alsVal, brightnessVal], idx) => {
                      // Find previous and next brightness levels
